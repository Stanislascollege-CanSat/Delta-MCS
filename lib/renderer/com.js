...
const com = {
  ...
  open: function(name,givenBaudRate) {
    ...
    missionConsole.error('%c[COM] %cSerialport already open.','color: #38006b','color: #000')
  },
  openPort: function() {
  ...
  closeEvent: function() {
    com.port.on('close', () => {
      console.info('%c[COM] %cport closed.','color: #38006b','color: #000')
      missionConsole.log('[COM] port closed.')
