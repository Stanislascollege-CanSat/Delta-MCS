...
const com = {
  ...
  list: function() {
    ...
    for (let i = 0; i < this.devices.length; i++) {
      ...
      console.log(this.devices[i].manufacturer)
  ...
  dataEvent: function() {
    com.port.on('data', (data) => {
      console.log('%c[COM] %cRecieved data from %c'+com.port.path+'%c: '+data,'color: #38006b','color: #000','color: #900','color: #000')
      missionConsole.log(data)
    })
  }
}
