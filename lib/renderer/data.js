...
const data = {
  ...
  packetDefinitions: [
    ...
    { code: 'SDP', name: 'statusDeployed', type: 'status'},
    // Functions.
    { code: 'F', name: 'Function', type: 'function' },
    // Tracking data return.
    { code: 'AL', name: 'BMP280 Altitude', type: 'retrunAltChart' },
    { code: 'GA', name: 'GPS Latitude', type: 'returnLatCounter' },
    { code: 'GO', name: 'GPS Longitude', type: 'returnLonCounter' },
    ...
    for (let i = 0; i < data.packetDefinitions.length; i++) {
      if (data.packetDefinitions[i].code === name) {
        ...
        switch (data.packetDefinitions[indexOfdataset].type) {
          ...
          case 'retrunAltChart':
            live.updateChart(data.packetDefinitions[indexOfdataset].code, value, time)
            // Send altitude to lambda.
            commands.send(`[AIM_ALT${value}]`)
            break
          case 'returnLatCounter':
            $(`data-${data.packetDefinitions[indexOfdataset].code}`).innerHTML = value
            // Send latitude to lambda.
            commands.send(`[AIM_LAT${value}]`)
            break
          case 'returnLonCounter':
            $(`data-${data.packetDefinitions[indexOfdataset].code}`).innerHTML = value
            // Send longitude to lambda.
            commands.send(`[AIM_LON${value}]`)
            break
        }
      } else {
